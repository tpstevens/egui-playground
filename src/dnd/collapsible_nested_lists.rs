...
pub struct CollapsibleNestedLists {
    name: String,
    root: Vec<ItemList>,
}
...
struct ItemList {
    ...
    // TODO: since dnd_children_idx_bounds only gets updated when children_collapsed is false...
    // all logic that uses it needs to check children_collapsed (because otherwise the bounds may
    // not be accurate). Is there a more reliable way to handle that state?
...
impl ItemList {
    ...
    fn can_insert(&mut self, dnd_idx: usize) -> bool {
        !self.children_collapsed
            && !self.dnd_dragging
            && self.dnd_children_idx_bounds.contains(dnd_idx)
    }
    ...
    fn remove_child_item(&mut self, dnd_idx: usize) -> Option<Self> {
        for i in 0..self.children.len() {
            ...
            if !child.children_collapsed && child.dnd_children_idx_bounds.contains(dnd_idx) {
            ...
            *dnd_idx,
            ...
            |ui, dnd_item| {
                dnd_item.ui(ui, |ui, handle, item_state| {
                    self.dnd_dragging = item_state.dragged;

                    ui.horizontal(|ui| {
                        handle.ui(ui, |ui| {
                            if self.children_collapsed || self.dnd_dragging {
                                ui.toggle_value(&mut self.children_collapsed, ">");
                            } else {
                                ui.toggle_value(&mut self.children_collapsed, "v");
                            }
                            ui.label(self.item.id.to_string());
                            ui.label(self.item.contents);
                        });
                    });
                })
            },
